**REPO CURRENTLY NOT USABLE - UPDATES BEING MADE AS OF 11/2025**
...
-   **00_QIIME2** - All sequence data goes through scripts 01-06, and separate analyses are carried out in Expt 1 and Expt 2 folders.
    ...
    -   **06_data_formatting.R** - R script that formats tables for easier use downstream.

    -   **07_summarystats.R** - R script that provides summary statistics for Expt1 and Expt2 16s data.
...
-   **Expt1** & **Expt2** - contain R scripts for 16s data analysis, including PCA, barplot visualizing, and linear modeling, separated by Expt1 (microcosm expt) and Expt2 (greenhouse expt).

-   **data** - contains the data used in analyses for both experiments. Note that ASV matrices are absent due to large file sizes.

-   **plots** - output plots for data visualization.