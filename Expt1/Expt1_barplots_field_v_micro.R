...
extra_cols <- which(
    grepl("-Expt1$", colnames(otu_mat)) & colnames(otu_mat) != "W-soil-Expt1"
)
...
extra_rows <- which(
    grepl("-Expt1$", rownames(samples_df)) &
        rownames(samples_df) != "W-soil-Expt1"
)
...
samples_df_filtered <- samples_df_filtered |>
    mutate(
        treatment = case_when(
            grepl("Dr-Expt1", treatment) ~ gsub(
                "Dr-Expt1",
                "A_DR-Expt1",
                treatment
            ),
            grepl("LR-Expt1", treatment) ~ gsub(
                "LR-Expt1",
                "A_LR-Expt1",
                treatment
            ),
            grepl("MP-Expt1", treatment) ~ gsub(
                "MP-Expt1",
                "A_MP-Expt1",
                treatment
            ),
            grepl("TF-Expt1", treatment) ~ gsub(
                "TF-Expt1",
                "A_TF-Expt1",
                treatment
            ),
            grepl("UM-Expt1", treatment) ~ gsub(
                "UM-Expt1",
                "A_UM-Expt1",
                treatment
            ),
            grepl("W-Expt1", treatment) ~ gsub(
                "W-Expt1",
                "A_W-Expt1",
                treatment
            ),
            grepl("KF-Expt1", treatment) ~ gsub(
                "KF-Expt1",
                "A_KF-Expt1",
                treatment
            ),
            grepl("ODR-Expt1", treatment) ~ gsub(
                "ODR-Expt1",
                "A_ODR-Expt1",
                treatment
            ),
...
full_top15_families <- otu_treatment_counts_full |>
    mutate(
        Family = ifelse(
            Family %in%
                top15_families$Family &
                !grepl("incertae sedis", Family, ignore.case = TRUE),
            Family,
            "Other"
        )
    )
...
kf_odr_plot_df <- kf_odr_plot_df |>
    mutate(
        ...
        treatment_label = fct_recode(treatment_label, "+" = "Y", "-" = "N")
    )
...
kf_odr_plot <- ggplot(
    kf_odr_plot_df,
    aes(x = treatment_label, y = Count, fill = Family)
) +
...
ggsave(
    "Expt1_kf_odr_combined_barplot.jpg",
    plot = kf_odr_combined_plot,
    width = 10,
    height = 6,
    dpi = 500
)
ggsave(
    "~/Library/CloudStorage/OneDrive-UniversityofNewHampshire/GreenManureProject/WRITING/plots/Expt1_kf_odr_combined_barplot.jpg",
    plot = kf_odr_combined_plot,
    width = 10,
    height = 6,
    dpi = 500
...
home_field_plot <- ggplot(
    ...
    facet_wrap(
        ...
        scales = "free_x",
        ncol = 6,
        labeller = labeller(geno = geno_labels)
    ) +
    theme_cowplot() +
    labs(x = "Treatment", y = "Relative Abundance", fill = "Bacterial Family") +
    theme_cowplot() +
...
ggsave(
    "Expt1_home_field_combined_barplot.jpg",
    plot = home_field_combined_plot,
    width = 14,
    height = 6,
    dpi = 500
)
ggsave(
    "~/Library/CloudStorage/OneDrive-UniversityofNewHampshire/GreenManureProject/WRITING/plots/Expt1_home_field_combined_barplot.jpg",
    plot = home_field_combined_plot,
    width = 14,
    height = 6,
    dpi = 500
