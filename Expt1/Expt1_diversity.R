...
shannon_div_df$micro <- factor(
    shannon_div_df$micro,
    levels = c("N", "H", "KF", "ODR")
)

# Update the cyano labels in the data
shannon_div_df$cyano <- factor(
    shannon_div_df$cyano,
    levels = c("N", "Y"),
    labels = c("No", "Yes")
)
...
shannon_div_df$x_axis <- paste(
    shannon_div_df$group,
    shannon_div_df$inoc,
    sep = "_"
...
Expt1_shan_div <- ggplot(
    shannon_div_df,
    aes(x = geno, y = ShannonDiversity, color = cyano)
) +
    stat_summary(
        fun = "mean",
        geom = "point",
