...
#set inoculum levels
pca_scores$inoculum <- factor(
    pca_scores$inoculum,
    levels = c("Home", "Kingman Farm", "Dairy Farm", "Uninoculated", "Field")
)

# Step 4: Visualize PCA
pca_plot <- ggplot(
    pca_scores,
    aes(x = PC1, y = PC2, color = inoculum, shape = cyano)
) +
    geom_point(size = 2.5) +
    scale_color_manual(name = "Microbiome Source", values = custom_colors) +
    scale_shape_manual(
